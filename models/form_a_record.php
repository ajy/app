...
class FormARecord extends AppModel {
	...
	function afterSave($created){
		if($created){// this entire thing basically keeps subject_memberships up to date every time a form is submitted
			Configure::load("feedback");
			$max = Configure::read('max_sub_num');//max is the max number of submissions possible
			$savedRecords = $this->query('select * from subject_memberships as SubjectMembership where SubjectMembership.form_a_submitted < "'.$max.'" and exists (select * from form_a_records as far where far.student = SubjectMembership.student_id and far.subject_id = SubjectMembership.subject_id and far.submission_number = '.$max.')');
			//The query basically finds all subject_memberships records that are not correctly stored
			$this->bindModel(
				array('hasMany' => array(
			...
			foreach($savedRecords as $savedRecord){
				$savedRecord['SubjectMembership']['form_a_submitted']=$max;//set to highest submission number for this student and subject
				$this->SubjectMembership->save($savedRecord);
			}//saves subject_memberships records correctly
		}
	}
	...
	function calcOverallFormAResults($user_id) {
		$userSubjectsData=$this->Subject->find('all', array(
			'recursive' => -1,
			'conditions' => array(
				'Subject.teacher1' => $user_id
		...
		$formAResults=null;
			...
			if($totalRows==null){
				...
				$ninetyPerAllRows=0.9*$totalRows;
				$requiredRecords=$this->find('all', array(
						'fields' => array('avg(q1)','avg(q2)','avg(q3)','avg(q4)','avg(q5)','avg(q6)','avg(q7)','avg(q8)','avg(q9)','avg(q10)'),
						'order' => array('avg(q1 + q2 + q3 + q4 + q5 + q6 + q7 + q8 + q9 + q10) ASC'),
						'limit' => floor($ninetyPerAllRows),
						'offset' => ceil(0.05*$totalRows),
						'conditions' => array(
							'subject_id' => $userSubject['id'],
						...
						'group' => array('FormARecord.submission_number'),
					)
				);
				$sumQ1=$sumQ2=$sumQ3=$sumQ4=$sumQ5=$sumQ6=$sumQ7=$sumQ8=$sumQ9=$sumQ10=0;
				foreach($requiredRecords as $requiredRecord){
	...
	function calcAllOverallFormAResults() {
		$allFormAResults = null;
		$allTeachers = $this->query("select id,username,name from users as User where group_id=2");
		$i=1;
		foreach($allTeachers as $teacher) {
			$teachersResults = $this->calcOverallFormAResults($teacher['User']['id']);
			if((!is_string($teachersResults))&&isset($teachersResults)) {
				foreach($teachersResults as $teachersResult) {
					if(!is_string($teachersResult)){
						$teachersResult['teacherUserName']= $teacher['User']['username'];
